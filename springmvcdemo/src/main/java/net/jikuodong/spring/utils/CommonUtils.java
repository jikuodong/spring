...
public class CommonUtils
{
	...
	public static void downloadDirectory(byte[] bytes, String dir) throws Exception
	{
		...
		{
			OutputStream out = new FileOutputStream(dir + "\\" + ((Map<?, ?>) list.get(i)).get("fileName"));
	...
	public static List<String> getNUm(String str){
	    str.trim();   //String number=lNum.substring(3, 4);//直接获取传递过来的字符串中的数字（适用于字符串中仅仅包含一个数字）
	 ...
	 * @param numEnd
	 * @return
	 * @date 2019年5月30日 下午2:17:59
	 */
	public static float generateFloat(double numStart, double numEnd)
	{
		Random random = new Random();

		float add = (float) (random.nextFloat() * (numEnd - numStart));
		float randomFloat = (float) (add + numStart);

		return randomFloat;
	}

	public static String getPercentage(String num1, String num2)
	{
		NumberFormat numberFormat = NumberFormat.getInstance();
        // 设置精确到小数点后2位
        numberFormat.setMaximumFractionDigits(2);
        double dou = (Double.valueOf(num1) - Double.valueOf(num2))/ Double.valueOf(num1)  * 100;
		return numberFormat.format(dou);
	}

	public static void main(String[] args)
	{
		List<String> dnaResult = CommonUtils.getNUm("但是复苏的异常细胞2，其中疑似病变细胞：0,3");
